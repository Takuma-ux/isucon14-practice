...
func appGetNotification(w http.ResponseWriter, r *http.Request) {
	...
	// status と chairs.total_rides_count を同じスナップショットで読む（TX 無しだと
	// ARRIVED 応答の直前に COMPLETED+count++ が入り CODE=33 になる）
	tx, err := db.Beginx()
	if err != nil {
		writeError(w, http.StatusInternalServerError, err)
		return
...
func calculateDiscountedFare(ctx context.Context, q sqlx.QueryerContext, userID string, ride *Ride, pickupLatitude, pickupLongitude, destLatitude, destLongitude int) (int, error) {
	var coupon Coupon
	discount := 0
	if ride != nil {
